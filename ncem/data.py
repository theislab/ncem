...
class DataLoaderMetabric(DataLoader):
    ...
    def _register_images(self, data_path: str):
        """
        ...
        """
        patient_col = 'metabricId'
        img_key_col = "ImageNumber"
        single_cell_all = read_csv(
            data_path + 'single_cell_data/single_cell_data.csv',
            usecols=[patient_col, 'core_id', img_key_col],
            dtype={patient_col: str, 'core_id': str, img_key_col: str}
        )
        single_cell = single_cell_all.copy()
        single_cell.drop_duplicates(inplace=True)
        single_cell['long'] = single_cell[patient_col] + '_' + single_cell['core_id'] + '_' + \
                              single_cell[img_key_col].astype(str)
        single_cell['long'] = single_cell['long'].str.replace('-', '')
        # Drop images with fewer than threshold cells:
        cell_count = pd.DataFrame({'count': single_cell_all.groupby(img_key_col).size()}).reset_index()
        img_ids_pass = set([
            x for x, y in zip(cell_count[img_key_col].values, cell_count["count"].values) if y >= 100
        ])
        # There are no tissues without HR+ or HR- labelled cells (tumor cells) above 100 cells observed total.
        single_cell = single_cell.iloc[np.where([x in img_ids_pass for x in single_cell[img_key_col].values])[0], :]
        self.img_key_to_fn = dict(single_cell[[img_key_col, 'long']].values)
        self.img_to_patient_dict = dict(single_cell[[img_key_col, patient_col]].values)

    def _register_nodes(self, data_path: str):
        """
        Register nodes based on segmentation masks.
        :param data_path: (str) path to data folder
        :return:
        """
        assert self.img_key_to_fn is not None, "register images first"
        img_key_col = "ImageNumber"
        self.nodes_by_image = {}
        single_cell = read_csv(data_path + 'single_cell_data/single_cell_data.csv',
                               usecols=[img_key_col, 'ObjectNumber'],
                               dtype={img_key_col: str, 'ObjectNumber': str})
        for k in self.img_key_to_fn.keys():
            # has missing values
            node_ids = list(single_cell[single_cell[img_key_col] == k]['ObjectNumber'])
            # # consecutive
            # node_ids = np.arange(1, len(node_ids) + 1)
            self.nodes_by_image[k] = [str(k) + '_' + str(i) for i in node_ids]

    def _load_node_positions(self, data_path: str):
        """
        Loads the position matrices from for each registered image.
        ...
        single_cell = read_csv(data_path + 'single_cell_data/single_cell_data.csv',
                               usecols=[img_key_col, 'Location_Center_X', 'Location_Center_Y'],
                               dtype={img_key_col: str, 'Location_Center_X': float, 'Location_Center_Y': float})
        for k in self.img_key_to_fn.keys():
            self.position_matrix[k] = np.array(
                single_cell[single_cell[img_key_col] == k][['Location_Center_X', 'Location_Center_Y']]
            )

    def _load_node_features(self, data_path: str):
        """
        ...
        single_cell = read_csv(
            data_path + 'single_cell_data/single_cell_data.csv',
            usecols=[img_key_col] + self.node_feature_names,
            dtype={img_key_col: str}
        )
        self.node_features = {}
        for k in self.img_key_to_fn.keys():
            self.node_features[k] = np.array(single_cell[single_cell['ImageNumber'] == k].drop(['ImageNumber'], axis=1))

    def _load_node_types(self, data_path: str):
        """
        ...
        single_cell = read_csv(
            data_path + 'single_cell_data/single_cell_data.csv',
            usecols=[img_key_col, 'ObjectNumber', 'description'],
            dtype={img_key_col: str, "ObjectNumber": str}
        )

        node_type_names = list(np.sort(np.unique(single_cell['description'].values)))
        node_types = {}
        for k in self.img_key_to_fn.keys():
            x = single_cell[single_cell[img_key_col] == k]['description'].values
            node_cluster_idx = np.array([  # index in encoding vector
                node_type_names.index(y) for y in x
            ])
            types_one_hot = np.zeros((x.size, len(node_type_names)))
            types_one_hot[np.arange(x.size), node_cluster_idx] = 1
            node_types[k] = types_one_hot

        self.node_types = node_types
        self.node_type_names = dict([(x, x) for x in node_type_names])
    ...
    def _load_node_groups(self, data_path: str):
        """
        Loads all levels of manual graph coarsening.
        :param data_path: (str) path to data folder
        :return:
        """
        self.node_groups = {}
        self.node_group_names = {}

    def _load_graph_features(
            self,
            data_path: str,
            label_selection: Union[List[str], None]
    ):
        """
        Loads the tissue metadata from the data.
        :param data_path: (str) path to data folder
        :param label_selection: (Union[List[str], None]) list of selected labels of graph level covariates
        :return:
        """
        patient_col = 'METABRIC.ID'
        disease_features = {
        ...
        usecols = label_cols_toread_csv + [patient_col] + ['Cohort', 'Date.Of.Diagnosis']
        tissue_meta_data = read_csv(
            data_path + 'single_cell_data/41586_2019_1007_MOESM7_ESM.csv',
            sep='\t',
            usecols=usecols
        )[usecols]
        tissue_meta_data.columns = label_cols_toread + [patient_col] + ['cohort', 'date_of_diagnosis']
        ...
        self.label_selection = list(label_cols.keys())
        self.continuous_mean = continuous_mean
        self.continuous_std = continuous_std
        self.survival_mean = survival_mean
        self.label_data_types = label_cols
        self._diseased_img_keys = list(self.img_to_patient_dict.keys())
        self._nondiseased_img_keys = []
        self.ref_img_keys = {}
