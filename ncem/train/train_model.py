...
from ncem.estimators import EstimatorLinear, EstimatorInteractions, EstimatorED, EstimatorEDncem, \
    EstimatorCVAE, EstimatorCVAEncem


class TrainModel:
    estimator: Union[EstimatorLinear, EstimatorInteractions, EstimatorED, EstimatorEDncem, EstimatorCVAE, EstimatorCVAEncem]

    def _try_save(self, fn: str, obj):
    ...
    def _save_evaluation(self, fn: str):
        evaluation_val = self.estimator.evaluate_any(
            img_keys=self.estimator.img_keys_eval,
            node_idx=self.estimator.nodes_idx_eval
        )
        evaluation_train = self.estimator.evaluate_any(
            img_keys=self.estimator.img_keys_train,
            node_idx=self.estimator.nodes_idx_train
        )
        evaluation_all = self.estimator.evaluate_any(
            img_keys=self.estimator.img_keys_all,
            node_idx=self.estimator.nodes_idx_all
        )
        evaluations = {
        ...
        if len(self.estimator.img_keys_test) > 0:
            evaluation_test = self.estimator.evaluate_any(
                img_keys=self.estimator.img_keys_test,
                node_idx=self.estimator.nodes_idx_test
            )
...
class TrainModelCVAEBase(TrainModel):
    ...
    def _save_evaluation_prior_sampling(self, fn: str):
        evaluation_val = self.estimator.evaluate_any_prior_sampling(
            img_keys=self.estimator.img_keys_eval,
            node_idx=self.estimator.nodes_idx_eval
        )
        evaluation_train = self.estimator.evaluate_any_prior_sampling(
            img_keys=self.estimator.img_keys_train,
            node_idx=self.estimator.nodes_idx_train
        )
        evaluation_all = self.estimator.evaluate_any_prior_sampling(
            img_keys=self.estimator.img_keys_all,
            node_idx=self.estimator.nodes_idx_all
        )
        evaluations = {
        ...
        if len(self.estimator.img_keys_test) > 0:
            evaluation_test = self.estimator.evaluate_any_prior_sampling(
                img_keys=self.estimator.img_keys_test,
                node_idx=self.estimator.nodes_idx_test
            )
            evaluations['test'] = evaluation_test
        self._try_save(fn + "_evaluation_prior_sampling.pickle", evaluations)

    def _save_evaluation_posterior_sampling(self, fn: str):
        evaluation_val, true_val, pred_val, \
        z_val, z_mean_val, z_log_var_val = self.estimator.evaluate_any_posterior_sampling(
            img_keys=self.estimator.img_keys_eval,
            node_idx=self.estimator.nodes_idx_eval
        )
        evaluation_train, true_train, pred_train, \
        z_train, z_mean_train, z_log_var_train = self.estimator.evaluate_any_posterior_sampling(
            img_keys=self.estimator.img_keys_train,
            node_idx=self.estimator.nodes_idx_train
        )
        evaluation_all, true_all, pred_all, \
        z_all, z_mean_all, z_log_var_all = self.estimator.evaluate_any_posterior_sampling(
            img_keys=self.estimator.img_keys_all,
            node_idx=self.estimator.nodes_idx_all
        )
        evaluations = {
        ...
        if len(self.estimator.img_keys_test) > 0:
            evaluation_test, true_test, pred_test, \
            z_test, z_mean_test, z_log_var_test = self.estimator.evaluate_any_posterior_sampling(
                img_keys=self.estimator.img_keys_test,
                node_idx=self.estimator.nodes_idx_test
            )
            evaluations['test'] = evaluation_test
...
class TrainModelCVAE(TrainModelCVAEBase):
    ...
    def _save_specific(self, fn):
        self._save_evaluation_prior_sampling(fn=fn)
