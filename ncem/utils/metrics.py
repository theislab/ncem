...
def r_squared_linreg(y_true, y_pred):
    ...
    def f0(): return tnp.zeros(shape=(), dtype=tf.float32)
    def f1(): return tnp.ones(shape=(), dtype=tf.float32)
    def r(): return ssxym / tnp.sqrt(xmym)  # formula for r
    def r2(): return r**2  # formula for r_squared
    # R-value
    # If the denominator was going to be 0, r = 0.0
    r = tf.cond(
        tnp.not_equal(xmym, tnp.zeros(shape=(), dtype=tf.float32)),
        r,
        f0
    )
    # Test for numerical error propagation (make sure -1 < r < 1)
    r_squared = tf.cond(
        tnp.greater(tnp.abs(r), tnp.ones(shape=(), dtype=tf.float32)),
        f1,
        r2
    )
    return r_squared
...
def logp1_r_squared_linreg(y_true, y_pred):
    ...
    x = tf.math.log(y_true + 1.)
    y = tf.math.log(y_pred + 1.)
