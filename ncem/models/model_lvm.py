...
from ncem.models.layers import Encoder, Decoder, \
    GaussianOutput, NegBinOutput, NegBinSharedDispOutput, NegBinConstDispOutput, SamplingPrior
...
class ModelLVM:
    def __init__(
        ...
        if probabilistic:
            ...
            logqz_x = -0.5 * tf.reduce_mean(tf.reduce_sum(
                tf.square(z - z_mean) * tf.exp(-z_log_var) +
                z_log_var + log2pi,
                axis=-1
            ))
            logpz = -0.5 * tf.reduce_mean(tf.reduce_sum(
                tf.square(z) + log2pi,
                axis=-1
            ))
            d_kl = logqz_x - logpz
